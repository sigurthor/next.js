...
export default class Router {
  ...
  onPopState(e) {
    ...
    const route = (e.state || {}).route || toRoute(pathname).cpath
    const params = toRoute(pathname).params
    ...
    Promise.resolve()
      .then(async () => {
        ...
        this.route = toRoute(pathname).cpath
        this.set(getURL(), {
          ...data,
          props
        })
  ...
  async fetchComponent(url) {
    const {cpath, path, params} = toRoute(parse(url).pathname)
    const route = cpath;
    let data = this.components[cpath]
    if (!data) {
      let cancel
